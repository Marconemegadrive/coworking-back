    ...
    async update(req: Request, res: Response) {
        ...
        try {
            ...
            if (num == 1) {
                ...
                res.send({
                    message: `Não foi possível ataulizar o Telefone com o id=${telefone.id}. O Telefone não foi encontrado, ou ele está vazio!`
                });
            }
        } catch (err) {
            res.status(500).send({
                message: `Error ao atualizar o Telefone com id=${telefone.id}.`
            });
        }
    }
