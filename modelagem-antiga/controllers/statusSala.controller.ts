...
import { StatusSalas } from "../models/statusSala";  // Certifique-se de importar a classe correta StatusSala
...
export default class StatusSalaController {
    ...
    async create(req: Request, res: Response) {
        ...
        try {
            const status : StatusSalas = req.body;
    ...
    async update(req: Request, res: Response) {
        let status: StatusSalas = req.body;
        ...
        try {
            ...
            if (num == 1) {
                ...
                res.send({
                    message: `Não foi possível ataulizar o StatusSala com o id=${status.id}. O StatusSala não foi encontrado, ou ele está vazio!`
                });
            }
        } catch (err) {
            res.status(500).send({
                message: `Error ao atualizar o StatusSala com id=${status.id}.`
            });
        }
    }
