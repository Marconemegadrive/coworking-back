...
export default class EmailController {
    ...
    async update(req: Request, res: Response) {
        ...
        email.id = parseInt(req.params.id);
        try {
            ...
            if (num == 1) {
                ...
                res.send({
                    message: `Não foi possível ataulizar o Email com o id=${email.id}. O Email não foi encontrado, ou ele está vazio!`
                });
            }
        } catch (err) {
            res.status(500).send({
                message: `Error ao atualizar o Email com id=${email.id}.`
            });
        }
    }
