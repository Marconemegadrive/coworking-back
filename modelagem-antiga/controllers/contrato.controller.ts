...
export default class ContratoController {
  ...
  async update(req: Request, res: Response) {
      ...
      contrato.id = parseInt(req.params.id);
      try {
          ...
          if (num == 1) {
              ...
              res.send({
                  message: `Não foi possível ataulizar o Contrato com o id=${contrato.id}. O Contrato não foi encontrado, ou ele está vazio!`
              });
          }
      } catch (err) {
          res.status(500).send({
              message: `Error ao atualizar o Contrato com id=${contrato.id}.`
          });
      }
  }
