...
export default class StatusClienteController {
    ...
    async update(req: Request, res: Response) {
        ...
        try {
            ...
            if (num == 1) {
                ...
                res.send({
                    message: `Não foi possível ataulizar o StatusCliente com o id=${status.id}. O StatusCliente não foi encontrado, ou ele está vazio!`
                });
            }
        } catch (err) {
            res.status(500).send({
                message: `Error ao atualizar o StatusCliente com id=${status.id}.`
            });
        }
    }
    ...
    async deleteAll(req: Request, res: Response) {
        try {
            const num = await statusClienteRepository.deleteAll();
            res.send({ message: `${num} StatusCliente foram deletados com sucesso!` });
        } catch (err) {
            res.status(500).send({
                message: "Algum erro ocorreu enquato deletava todos os StatusCliente."
            });
        }
    }
