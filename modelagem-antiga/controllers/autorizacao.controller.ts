...
export default class AutorizacaoController {
  ...
  async update(req: Request, res: Response) {
      ...
      autorizacao.id = parseInt(req.params.id);
      try {
          ...
          if (num == 1) {
              ...
              res.send({
                  message: `Não foi possível ataulizar o autorizacao com o id=${autorizacao.id}. O autorizacao não foi encontrado, ou ele está vazio!`
              });
          }
      } catch (err) {
          res.status(500).send({
              message: `Error ao atualizar o autorizacao com id=${autorizacao.id}.`
          });
      }
  }
