  ...
  async update(req: Request, res: Response) {
      ...
      try {
          ...
          if (num == 1) {
              ...
              res.send({
                  message: `Não foi possível ataulizar o Endereco com o id=${endereco.id}. O Endereco não foi encontrado, ou ele está vazio!`
              });
          }
      } catch (err) {
          res.status(500).send({
              message: `Error ao atualizar o Endereco com id=${endereco.id}.`
          });
      }
  }
