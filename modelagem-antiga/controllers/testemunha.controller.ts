...
export default class TestemunhaController {
    ...
    async update(req: Request, res: Response) {
        ...
        testemunha.id = parseInt(req.params.id);
        try {
            ...
            if (num == 1) {
                ...
                res.send({
                    message: `Não foi possível ataulizar o Testemunha com o id=${testemunha.id}. O Testemunha não foi encontrado, ou ele está vazio!`
                });
            }
        } catch (err) {
            res.status(500).send({
                message: `Error ao atualizar o Testemunha com id=${testemunha.id}.`
            });
        }
    }
